...
STATION_ID   = "ST_006"
STATION_NAME = "falsification"
STATION_DESC = "Test claims for falsifiability, generate kill conditions"
...
def choose_nlp(path: Path, cfg: dict[str, Any]) -> dict[str, Any]:
    return {
        "nlp_id": cfg.get("nlp_id", "llm_reasoning"),
        "nlp_path": MODELS / cfg.get("model_folder", "llm_reasoning"),
...
def process_one(path: Path, nlp_info: dict, cfg: dict[str, Any],
    ...
    try:
        obj=_read_input(path); data=obj.get("data",obj); claims=data.get("load_bearing", data.get("claims",[]))
        ...
        for c in claims:
            ...
            llm=_api("generate", {"model":"phi4", "prompt":prompt})
