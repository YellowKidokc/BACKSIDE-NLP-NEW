...
STATION_ID   = "ST_005"
STATION_NAME = "load-bearing-claims"
STATION_DESC = "Identify structurally load-bearing claims vs rhetoric/narrative/metadata"
...
def choose_nlp(path: Path, cfg: dict[str, Any]) -> dict[str, Any]:
    return nlp_route(API_BASE, MODELS, cfg, "zero_shot_llm", "classify")
...
def process_one(path: Path, nlp_info: dict, cfg: dict[str, Any],
    ...
    try:
        obj=read_input(path); claims=obj.get("data",obj).get("classified_claims", obj.get("data",obj).get("claims",[]))
