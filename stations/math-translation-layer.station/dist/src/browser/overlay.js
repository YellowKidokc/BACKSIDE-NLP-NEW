...
exports.enhanceMathElement = enhanceMathElement;
...
function findReviewedTranslation(source, document) {
...
function renderMathJax(element, markup) {
...
function attachToggle(state, shell) {
...
function enhanceMathElement(element) {
    ...
    const spoken = (0, core_1.translate)({
        input: cleanedSource,
        format: "tex",
...
function enhanceDocument(document = window.document) {
    ensureStyles(document);
    const selected = Array.from(document.querySelectorAll(".eq-block, .equation-block, .equation-block .math, .math-box, .math, .hero-eq, .bx-eq, script[type^='math/tex'], [data-tex], mjx-container")).filter((element, index, array) => array.indexOf(element) === index);
    const elements = selected.filter((element) => !selected.some((candidate) => candidate !== element && candidate.contains(element)));
