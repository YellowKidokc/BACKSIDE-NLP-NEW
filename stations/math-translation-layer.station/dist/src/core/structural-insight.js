...
function segmentByOperators(children, operators) {
    const segments = [];
    let current = [];
...
function multiplicationSegments(children) {
...
function readableNode(node) {
    ...
    return (0, ast_utils_1.nodeSource)(node).replace(/\\/g, "");
}
...
function containsKind(nodes, kind) {
    return nodes.some((node) => {
        let found = false;
        (0, ast_utils_1.walkMath)(node, (current) => {
...
function generateInsight(ast) {
    ...
    if (productSegments.length >= 3) {
        ...
        sentences.push(`This equation is a chain where every link must hold. ${joinLabels(labels)} all multiply together, which means if any one factor drops to zero, the entire output collapses with it.`);
    ...
    if (subtractionSegments.length === 2) {
        ...
        const derivativeSquare = findFirst(leftTerm, (node) => node.kind === "superscript" &&
