...
def compute_truth_score(profile: ClaimProfile) -> ScoreResult:
    ...
    # Evidence floor: empirical/mechanistic claims cannot be promoted by posture,
    # grounding, combat survival, or cross-domain multiplier when support collapses.
    evidence_bound_claims = {
        "CAUSAL", "MECHANISTIC", "PREDICTIVE", "ONTOLOGICAL",
        "EXISTENTIAL", "DESCRIPTIVE", "CORRELATIONAL", "CONSTITUTIVE",
    }
    if profile.claim_type in evidence_bound_claims:
        if result.E <= 0.0:
            result.T_final = min(result.T_final, 0.40)
            result.caps_applied.append("NO_EVIDENCE_CAP_0.40")
        elif result.E < 0.20:
            result.T_final = min(result.T_final, 0.50)
            result.caps_applied.append("LOW_EVIDENCE_CAP_0.50")
        elif result.E < 0.35:
            result.T_final = min(result.T_final, 0.60)
            result.caps_applied.append("THIN_EVIDENCE_CAP_0.60")
