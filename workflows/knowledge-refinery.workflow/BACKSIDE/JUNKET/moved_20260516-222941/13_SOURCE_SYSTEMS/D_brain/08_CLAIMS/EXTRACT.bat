...
set "PYTHON=python"
set "EXTRACT=D:\brain\08_CLAIMS\extract.py"
set "EXPORT=D:\brain\08_CLAIMS\export_excel.py"
set "TARGETS=D:\brain\08_CLAIMS\targets.txt"
set "OUTPUT_DIR=D:\brain\08_CLAIMS\_OUTPUT"
set "DESKTOP=C:\Users\lowes\Desktop"
...
if not exist "%TARGETS%" (
    ...
    echo # \\192.168.1.177\Desktop\faiththruPhysics>> "%TARGETS%"
    echo #>> "%TARGETS%"
...
echo  [6] Full pipeline (extract all targets + export Excel)
echo  [0] Exit
echo.
set /p CHOICE="  Choose: "
...
if defined LAST_JSON (
    ...
    if /i "!DOEXPORT!"=="y" (
        %PYTHON% "%EXPORT%" "!LAST_JSON!" --output "%DESKTOP%\CLAIMS_REVIEW.xlsx"
        echo.
        echo  Excel saved to: %DESKTOP%\CLAIMS_REVIEW.xlsx
    )
)
echo.
pause
...
set /p OUTNAME="  Output filename [CLAIMS_REVIEW.xlsx]: "
if "%OUTNAME%"=="" set "OUTNAME=CLAIMS_REVIEW.xlsx"
%PYTHON% "%EXPORT%" "!LAST_JSON!" --output "%DESKTOP%\%OUTNAME%"
echo.
echo  Saved to: %DESKTOP%\%OUTNAME%
pause
goto :eof
...
:full_pipeline
echo.
echo  FULL PIPELINE: Extract all targets + Export Excel
echo  ============================================
...
if defined LAST_JSON (
    ...
    echo  Excel: %DESKTOP%\CLAIMS_REVIEW.xlsx
    echo  ============================================
)
echo.
pause
