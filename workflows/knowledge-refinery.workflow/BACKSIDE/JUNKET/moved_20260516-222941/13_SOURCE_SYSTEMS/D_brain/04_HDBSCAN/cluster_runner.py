...
def _run_postgres(cfg: dict, log: logging.Logger) -> int:
    ...
    report_path = Path(summ.get("report_path", str(LOG_DIR / "hdbscan_summary.csv")))
...
def _run_files(cfg: dict, log: logging.Logger) -> int:
    ...
    npz = Path(fs["input_npz"]) if fs.get("input_npz") else None
    out_dir = Path(fs["output_dir"]) if fs.get("output_dir") else None
    if not npz or not out_dir:
        log.error("files mode requires files_source.input_npz and files_source.output_dir")
        return 1
    if not npz.exists():
