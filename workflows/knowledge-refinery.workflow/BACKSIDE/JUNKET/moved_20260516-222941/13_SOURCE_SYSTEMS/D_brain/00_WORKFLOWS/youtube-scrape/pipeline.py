...
ROOT = HERE.parent.parent  # D:\brain
LOG_DIR = ROOT / "_LOGS"

# Add tool folders to sys.path.
for tool in ("01_WHISPER", "02_SBERT", "03_DEBERTA", "04_HDBSCAN", "05_YOUTUBE", "07_POSTGRES"):
    p = ROOT / tool
...
def _load_runner_config(tool_dir: str) -> dict:
    return json.loads((ROOT / tool_dir / "config.json").read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    if steps.get("scrape", True):
        ...
        try:
            ...
            yt_cfg = _load_runner_config("05_YOUTUBE")
            yt_cfg["table"] = table
    ...
    if steps.get("embed", True):
        ...
        try:
            ...
            sb_cfg = _load_runner_config("02_SBERT")
    ...
    if steps.get("classify", True):
        ...
        try:
            ...
            db_cfg = _load_runner_config("03_DEBERTA")
    ...
    if steps.get("cluster", True):
        ...
        try:
            ...
            cl_cfg = _load_runner_config("04_HDBSCAN")
